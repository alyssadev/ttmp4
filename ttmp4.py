...
def index():
    ...
    if not url:
        return "<form><input type=text name=url><input type=submit></form>"
    ...
    if not isfile("static/" + url.split("/")[-1] + ".mp4"):
        print(url)
        ytdl = Popen(["yt-dlp", "-f", "download_addr-0", "-o", "static/%(display_id)s.%(ext)s", url], stdout=PIPE)
...
if __name__ == "__main__":
    app.run(debug=True, port=51761, host="0.0.0.0")